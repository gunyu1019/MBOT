...
class Command:
    ...
    @_command.command(name="뮤트", permission=1)
    async def mute(self, ctx: Union[ApplicationContext, MessageCommand]):
        return

    @_command.command(name="뮤트해제", permission=1)
    async def unmute(self, ctx: Union[ApplicationContext, MessageCommand]):
        return
